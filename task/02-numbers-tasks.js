...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    let skl = (x1 * x2) + (y1 * y2)
    let modX = Math.sqrt(Math.pow((x1),2) + Math.pow((y1),2));
    let modY =  Math.sqrt(Math.pow((y2),2) + Math.pow((x2),2));
    return Math.acos(skl / (modX * modY));
}
