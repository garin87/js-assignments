...
function getRectangleString(width, height) {
  ...
  return String.fromCharCode(9484) + String.fromCharCode(9472).repeat(width - 2) 
                                   + String.fromCharCode(9488) + "\n" 
                                   + (String.fromCharCode(9474) + " ".repeat(width - 2) 
                                   + String.fromCharCode(9474) + "\n").repeat(height - 2)
                                   + String.fromCharCode(9492) + String.fromCharCode(9472).repeat(width-2) 
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < newStr.length; i++) {
    if(alphabet.indexOf(newStr[i]) > -1) t.push(rot13[alphabet.indexOf(newStr[i])]);
    else t.push(newStr[i]);
  }
...
function getCardId(value) {
  ...
  for (var i = 0; i < deck.length; i++) {
      if(deck[i] == value) return deck.indexOf(deck[i]);
  }
